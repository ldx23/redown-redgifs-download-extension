...
function addButton(vid_sidebar) {
	...
	const video = vid_sidebar.parentNode
	...
	mutations.forEach(mutation => {
		...
		if (mutation.addedNodes) {
			...
			for (var i = mutation.addedNodes.length - 1; i >= 0; i--) {
				...
				if (node.classList && node.classList.contains("Player-SideBarWrap")) {
					// console.log("FOUND")
					// console.log(node)
					addButton(node);
				}
		...
		if (mutation.removedNodes) {
			for (var i = mutation.removedNodes.length - 1; i >= 0; i--) {
				...
				if (node.classList && node.classList.contains("TapTracker")) {
					const video = node.getElementsByClassName("Video")[0]?.getElementsByTagName("video")[0];
					if (!video) return;
